...
read -r jobSm2PrivateKey jobSm2PublicKey < <(
  kubectl -n default run bk-job-keypair --rm -i --quiet --restart=Never --image hub.bktencent.com/blueking/bk-job-keypair -- python sm2_keypair/generate_sm2_keypair.py </dev/null 2>/dev/null | \
   jq -r '."job.encrypt.sm2PrivateKey" + " " + ."job.encrypt.sm2PublicKey"'
)
read -r jobPrivateKeyBase64 jobPublicKeyBase64 < <(
  kubectl -n default run bk-job-keypair-2 --rm -i --quiet --restart=Never --image hub.bktencent.com/blueking/bk-job-keypair -- python service-rsa-keypair/generate_service_rsa_keys.py </dev/null 2>/dev/null | \
