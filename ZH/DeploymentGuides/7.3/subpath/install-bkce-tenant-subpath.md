...
```bash
tenant_user='yanshou' # 这里管理员用户请根据自己环境修改
tenant_supermanager_userid=$(kubectl -n blueking exec bk-mysql8-0 -- mysql -uroot -pblueking bkuser -NsBe "select tenant_user_id from tenant_tenantuseridrecord where code=\"$tenant_user\" and tenant_id='system';")
...
# 进入容器执行 python manage.py iam_migrate 和 python manage.py apigw_migrate 命令
kubectl -n bkapp-cw0us0aitsm-prod exec -it deploy/cw0us0aitsm--web -- /app/.heroku/python/bin/python manage.py iam_migrate
kubectl -n bkapp-cw0us0aitsm-prod exec -it deploy/cw0us0aitsm--web -- /app/.heroku/python/bin/python manage.py apigw_migrate 
```
