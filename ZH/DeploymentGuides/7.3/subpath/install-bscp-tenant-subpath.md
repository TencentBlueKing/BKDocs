...
./scripts/add_user_desktop_app.sh -u $tenant_supermanager_userid -a 'bk_bscp' # 现有用户添加桌面应用
# 设为默认应用
...
需要配置域名 `$BK_DOMAIN` ，操作步骤已经并入 《部署步骤详解 —— 后台》 文档 的 “[配置用户侧的 DNS](manual-install-bkce.md#hosts-in-user-pc)” 章节。

配置成功后，可以通过域名 `$BK_DOMAIN/bscp` 访问首页，也可在桌面打开 “服务配置中心” 应用。
