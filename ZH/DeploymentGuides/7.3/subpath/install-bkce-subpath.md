...
|-----------------------------|----------------------------------------|-----------------|
| bk-repo                     | 3.2.3-beta.1                           | v3.2.3-beta.1      |
| bk-auth                     | 1.1.0-alpha.3                          | 1.1.0-alpha.3           |
| bk-apigateway               | 1.20.0-beta.1                        | 1.20.0-beta.1          |
| bk-user                     | 3.0.1-alpha.93                         | 3.0.1-alpha.93          |
| bk-iam                      | 1.14.0-alpha.6                     | v1.14.0-alpha.6        |
| bk-ssm                      | 1.1.0-alpha.4                          | 1.1.0-alpha.4          |
| bk-console                  | 0.3.0-beta.9                           | v0.3.0-beta.9           |
| bk-iam-saas                 | 1.12.0-alpha.36                        | v1.12.0-alpha.36        |
| bk-iam-search-engine        | 0.1.5                                  | v1.1.5        |
| bk-gse                      | v2.1.6-beta.63                         | v2.1.6-beta.63           |
| bk-cmdb                     | 3.16.1-feature-tenant-alpha25          | 3.15.1-feature-tenant-alpha25          |
| bkpaas-app-operator         | 1.7.0-alpha.63                         | v1.7.0-alpha.63          |
| bk-paas                     | 1.7.0-alpha.63                         | v1.7.0-alpha.63         |
| bk-applog                   | 1.1.17                                 | 1.1.17          |
| bk-ingress-nginx            | 1.3.5                                  | 1.3.1   |
| bk-ingress-rule             | 0.0.4                                  | 0.0.4           |
| bk-job                      | 9.9.9-codev.6                        | 9.9.9-codev.6           |
| bk-nodeman                  | 2.4.8-pre-alpha.2087                  | 2.4.8-pre-alpha.2087           |
| bk-sops                     |                    |   3.35.1-alpha.1.2129         |
| cw-aitsm                    |                    | 4.8.103-rc.421           |
| bk-cmsi                     |                    |   1.1.7         |
| bk-notice                   |                    |  1.6.1-alpha.9.353          |
| bcs-services-stack          | 1.31.0-alpha.20-tenant                  | v1.31.0-alpha.20-tenant           |
...
apps:
  bkappFilebeat:
    containersLogPath: "${containersLogPath}" # 也可以到 node 上执行 containerd config dump
...
单租户需要启用 bkiam ingress
```bash
cd $INSTALL_DIR/blueking/  # 进入工作目录
yq -i '.ingress.enabled=true' environments/default/subpath/bkiam-subpath-values.yaml.gotmpl
```
...
```bash
cd ${INSTALL_DIR:-~/bkce7.3-install}/blueking && ./scripts/dns-helper.sh $(ssh $(kubectl get pods -A -l app.kubernetes.io/name=ingress-nginx -o jsonpath='{.items[0].status.hostIP}') 'curl -s ip.sb') | awk 'NR==1 || $2 ~ /bkrepo|docker|helm|bk-apicheck/'
```
...
```bash
kubectl exec -n blueking deploy/bkiam-saas-web bkiam-saas -c bkiam-saas -- python manage.py sync_organization
```
...
```bash
# 这里管理员用户请根据自己环境修改
supermanager='yanshou' # 这里管理员用户名请根据自己环境修改
supermanager_userid=$(kubectl -n blueking exec bk-mysql8-0 -c mysql -- mysql -uroot -pblueking bkuser -NsBe "select tenant_user_id from tenant_tenantuseridrecord where code=\"$supermanager\";")
# 把上述环境变量写入 bashrc
bashrc=$HOME/.bashrc
...
```bash
./scripts/bk-admin.sh grant ${supermanager_userid} iam # 授权权限中心管理员
./scripts/bk-admin.sh su ${supermanager_userid} paas # 授权开发者中心管理员
./scripts/bk-admin.sh su ${supermanager_userid} bkrepo # 授权制品库，该步骤需要用户登录过制品库才行
./scripts/bk-admin.sh grant ${supermanager_userid} gw ALL # 授权 API 网关
```
...
授权网关给当前管理员用户
```bash
./scripts/bk-admin.sh grant "${supermanager_userid}" gw ALL # 也可将ALL 改为 bk-notice
```

进入API网关页面找到 `bk-notice` 网关配置插件 `http://${BK_DOMAIN}/apigw`
...
```bash
# 语音
./scripts/bk-admin.sh cmsi system voice '{"qcloud_app_id": "sss", "qcloud_app_key": "ssss"}'
# 企业微信
./scripts/bk-admin.sh cmsi system weixin '{"corp_id": "sss", "to_user": "", "wx_type": "qy", "agent_id": "4", "wx_token": "", "wx_app_id": "", "wx_secret": "", "corp_secret": "sss", "wx_template_id": ""}'
# 短信
./scripts/bk-admin.sh cmsi system sms '{"qcloud_app_id": "ss", "qcloud_app_key": "ss", "qcloud_sms_sign": "【腾讯蓝鲸】"}'
# 邮件
./scripts/bk-admin.sh cmsi system mail '{"smtp_pwd": "ss", "smtp_host": "smtp.exmail.qq.com", "smtp_port": 465, "smtp_user": "bk_sender@bktencent.com", "mail_sender": "bk_sender@bktencent.com", "smtp_usessl": True, "smtp_usetls": ""}'
```
...
```bash
cd $INSTALL_DIR/blueking
./scripts/setup_bkce7.sh -i bk_itsm
```
...
```bash
bkdl-7-devel.sh -ur 7.3.0-alpha nm_gse_full # 下载最新版 gse 以及插件包
...
### 权限中心授权管理员
蓝鲸 7.3 默认没有 admin 用户，即创建的现有用户需要在权限中心手动授权为管理员。
...
外网回调地址 和 内网回调地址 一样：
http://$BK_DOMAIN/bknodeman/backend

Agent包服务器目录: 保持为空
Agent包URL  : 2 个输入框均改为下面的内容：
http://$BK_DOMAIN/bkrepo/generic/blueking/bknodeman/data/bkee/public/bknodeman/download
...
继续部署（可并行）：
- [部署监控日志套餐](./install-co-suite-tenant-subpath.md)（建议先部署容器管理平台）