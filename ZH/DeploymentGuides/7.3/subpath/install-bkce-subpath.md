...
```bash
./scripts/bk-tenant-admin.sh grant "$supermanager_userid" bkapp bk_cmdb_saas bk_sops bk_cmsi cw_aitsm bk_notice
```
...
```bash
./scripts/add_user_desktop_app.sh -u $supermanager_userid -a 'bk_sops,cw_aitsm,bk_cmdb_saas,bk_notice' # 现有用户添加桌面应用
...
前面在部署完 cmdb_saas 后确认已经设置好业务公共变量（BCS依赖）
```bash
# 检查
