...
cd $INSTALL_DIR/blueking/  # 进入工作目录
bkci_custom_values=./environments/default/bkci/bkci-custom-values.yaml.gotmpl
yq -i ".config.bkCiPlatformManager=\"${tenant_supermanager_userid}\"" "$bkci_custom_values"
```
...
for f in ../ci-plugins/*.zip; do
  ...
  kubectl exec -i -n blueking deploy/bk-ci-bk-ci-store -c store -- \
    curl -s \
      http://127.0.0.1/api/op/pipeline/atom/deploy/"?publisher=admin" \
      -H "X-DEVOPS-UID: $tenant_supermanager_userid" -H "X-TENANT-ID: system" -F atomCode=$atom -F "file=@-;filename=$filename" < "$f" | jq .
...
桌面打开 “持续集成平台-蓝盾” 应用或者通过域名访问 `$BK_DOMAIN/devops` 。
...
| `BK_CODECC_FRONTEND_PUBLIC_URL` | `http://codecc.bkce7.bktencent.com` （如果有自定义域名，请自行修改。） | 全部 | |
| `BK_CI_FRONTEND_PUBLIC_URL` | `http://devops.bkce7.bktencent.com` （如果有自定义域名，请自行修改。） | 全部 | |
