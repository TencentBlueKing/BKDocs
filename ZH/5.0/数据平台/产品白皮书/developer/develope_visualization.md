...
业务数据都已经接入到数据平台，同时进行了实时计算、离线计算，并将数据保存到数据平台中。需要将 **数据可视化**，通过 **视图的形式** 呈现出来。

本文介绍从数据平台获取一个 IP 的进程(pid)的 CPU 使用率数据来绘制曲线图。
...
sql = "select dtEventTime,cpu_usage_pct as count,pid from table_name where ip='0.0.0.0' group by pid,dtEventTime order by dtEventTime limit 100000"
data = json.dumps({
...
以上例子从数据平台结果表中查询数据，然后构造绘制 echart 曲线图所需的数据格式，最后呈现出一个 IP 下主要进程 pid 的 CPU 使用情况，结果如下：
