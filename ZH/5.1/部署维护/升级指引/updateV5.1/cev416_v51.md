...
- 更新 MySQL
  ...
  - 恢复备份数据
    ...
    - Check步骤： 确认当前 Job 库编码是否为 UTF8。
      - 将 Job 库的字符集变更为 UTF8，此步骤适用于从低于 V4.1.16 升级上来的老用户,由于早期 Job 库是 latin 1 字符集
...
## 附录
- 如何手动创建 MongoDB.key文件
```bash
# 登陆到 MongoDB 机器
openssl rand -base64 756 > /data/bkee/public/mongodb/mongod.key
chmod 400 /data/bkee/public/mongodb/mongod.key
```
