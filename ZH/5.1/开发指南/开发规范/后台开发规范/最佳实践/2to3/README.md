...
1. 使用 patch 命令生成文件差异
        ...
        # 解析 test.py ，并将转换到py3后的文件差异输出到 test.patchplainplainplainplain
...
2. 直接把修改写回原文件
        ...
        # 除非传入了 `-n` 参数，否则会为原始文件创建一个副本plainplainplainplainplainplainplainplainplain
        2to3 -w test.py

3. 将整个项目代码转换到 py3

        2to3 -w -n myprojectplainplainplain
