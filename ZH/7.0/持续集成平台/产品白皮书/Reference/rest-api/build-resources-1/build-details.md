...
| HTTP代码 | 说明 | 参数类型 |
| :--- | :--- | :--- |
| 200 | successful operation | [数据返回包装模型构建详情-构建信息](build-details.md) |
...
| 参数名称 | 参数类型 | 必须 | 参数说明 |
| :--- | :--- | :--- | :--- |
| data | [构建详情-构建信息](build-details.md) | 否 | 数据 |
...
| model | [流水线模型-创建信息](build-details.md) | 是 | Build Model |
| id | string | 是 | 构建ID |
...
| name | string | 是 | 名称 |
| stages | List&lt;[流水线模型-阶段](build-details.md)&gt; | 是 | 阶段集合 |
...
| checkIn | [StagePauseCheck](build-details.md) | 否 | 当前Stage是否能重试 |
| customBuildEnv | object | 否 | 用户自定义环境变量 |
| finally | boolean | 否 | 标识是否为FinallyStage，每个Model只能包含一个FinallyStage，并且处于最后位置 |
| name | string | 是 | 阶段名称 |
| containers | List&lt;[流水线模型-多态基类](build-details.md)&gt; | 是 | 容器集合 |
| id | string | 否 | 阶段ID |
| stageControlOption | [StageControlOption](build-details.md) | 是 | 流程控制选项 |
| checkOut | [StagePauseCheck](build-details.md) | 否 | 当前Stage是否能重试 |
...
| reviewParams | List&lt;[人工审核-自定义参数](build-details.md)&gt; | 否 | reviewParams |
| manualTrigger | boolean | 否 | manualTrigger |
| checkTimes | integer | 否 | checkTimes |
| reviewDesc | string | 否 | reviewDesc |
| reviewGroups | List&lt;[Stage审核组信息](build-details.md)&gt; | 否 | reviewGroups |
...
| options | List&lt;[人工审核-自定义参数-下拉框列表剑](build-details.md)&gt; | 否 | 下拉框列表 |
...
| params | List&lt;[人工审核-自定义参数](build-details.md)&gt; | 否 | 审核传入变量 |
...
| elements | List&lt;[Element](build-details.md)&gt; | 否 | elements |
| name | string | 否 | name |
| id | string | 否 | id |
...
| additionalOptions | [ElementAdditionalOptions](build-details.md) | 否 | additionalOptions |
...
| timeout | integer | 否 | timeout |
| customVariables | List&lt;[NameAndValue](build-details.md)&gt; | 否 | customVariables |
| otherTask | string | 否 | otherTask |
| customEnv | List&lt;[NameAndValue](build-details.md)&gt; | 否 | customEnv |
...
| elementPostInfo | [元素post信息](build-details.md) | 否 | elementPostInfo |
