...
![](../7.0/assets/bk_nodeman-plugin-list.png)
...
其中，`zk配置` 的填写要求如下。
* 集群地址：IP 填写 **任意 k8s node IP**，PORT 填写 `32181`（注意不是默认的 `2181`，此处取 `bk-zookeeper` 服务的 NodePort）。
* 用户名和密码：默认无需修改。
...
cat <<EOF
Btfileserver: 内网: $file_server    外网: $file_server
Dataserver  : 内网: $data_server    外网: $data_server
Taskserver  : 内网: $task_server    外网: $task_server

外网回调地址:   http://$bknodeman_bapi_host:30300/backend
内网回调地址:   http://$bknodeman_bapi_host:30300/backend
...
点击 “测试 Server 及 URL 可用性”。测试通过后，可以点击“下一步”。
...
内网回调地址:   http://bknodeman.$BK_DOMAIN/backend
...
因为蓝鲸智云 v6 和 v7 仅为部署形态差异，各产品软件版本会保持一致。故请参考 《[安装蓝鲸 Agent（直连区域）](./ZH/7.0/节点管理/产品白皮书/QuickStart/DefaultAreaInstallAgent.md)》 文档安装 agent。
...
当安装 agent 完成后，你可以参考 《[插件管理](./ZH/7.0/节点管理/产品白皮书/Feature/Plugin.md)》 文档为所有 agent 批量安装 `bkmonitorbeat` 和 `bkunifylogbeat` 插件，以便使用监控及日志功能。
