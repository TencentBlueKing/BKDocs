...
## 前提条件
在配置平台中[新建业务](6.0/配置平台/快速入门/case1.md) 及业务拓扑。
...
![-w1300](../assets/15625715311134.jpg)
...
### 将进程绑定模块
[如何管理主机](6.0/bk_solutions/CD/CMDB/CMDB_management_hosts.md#分配主机到业务模块) 提到依据业务架构来划分业务拓扑，业务拓扑中模块代表服务，而服务将由一个或多个进程监听的端口来对用户或其他模块提供服务。

所以，需要将`进程`绑定至对应的`模块`上。

这里将`MariaDB`进程绑定至存储层模块上。

![-w1273](../assets/15625579982411.jpg)


### 监控系统自动实现进程端口监控

给模块 [分配主机](6.0/配置平台/快速入门/case1.md)

![-w1541](../assets/15625580245427.jpg)

等候一分钟，在蓝鲸自带的监控系统 [蓝鲸监控](6.0/蓝鲸监控/产品简介/README.md) 中可以看到 [进程的运行情况](6.0/蓝鲸监控/产品功能/Process_monitor_desc.md)。
